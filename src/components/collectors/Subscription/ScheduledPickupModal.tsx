                        ...
                        (scheduledPickups && scheduledPickups.filter(pickup => pickup.status === tab).map(pickup => (
                            <div className='border border-gray-700 p-3 rounded-lg mb-2 shadow-sm'>
                                <div className={`${pickup.status === 'pending' ? 'flex justify-between' : ''}`}>
                                    ...
                                    {pickup.status === 'pending' && <button disabled={!isToday(pickup.scheduledDate.toString())} onClick={() => handleNavigation(pickup._id)} className={`px-3 py-0.5 flex items-center bg-accent rounded-md ${isToday(pickup.scheduledDate.toString()) ? 'cursor-pointer' : 'opacity-50 cursor-not-allowed'}`}>
                                        <CheckCircle className='inline w-4 h-4 mr-1' />Complete</button>}
                                </div>
